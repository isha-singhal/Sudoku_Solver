...
// returns whether or not "number" has been used in particular subgrid
bool usedInSubgrid(int grid[N][N], int row, int col, int number) {
  int subgridStartRow = row - row % 3;
  int subgridStartCol = col - col % 3;
  for (int ii = 0; ii < 3; ii++)
    for (int jj = 0; jj < 3; jj++)
      if (grid[subgridStartRow + ii][subgridStartCol + jj] == number)
        return true;
  return false;
}
...
bool isValid(int grid[N][N], int row, int col, int number) {
  return !usedInRow(grid, row, number) &&
       ...
       !usedInSubgrid(grid, row, col, number);
}
